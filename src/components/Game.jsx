...
const Game = () => {
  ...
  const letterPixelsRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const ontouchmove = (ev) => {
      ...
      const x = Math.round(touch.clientX - canvas.getBoundingClientRect().left);
      const y = Math.round(touch.clientY - canvas.getBoundingClientRect().top);
      if (mousedownRef.current) {
        paint(x, y);
      }
      ev.preventDefault();
  ...
  const init = () => {
    ...
    letterPixelsRef.current = getPixelAmount(200, 200, 250);
  };
  ...
  const pixelThreshold = () => {
    if (getPixelAmount(0, 0, 50) / letterPixelsRef.current > 0.7) {
  ...
  const paint = (x, y) => {
    ...
    if (colour.a === 0) {
      ...
      const interpolate = (x0, y0, x1, y1, step) => {
        const points = [];
        const dx = x1 - x0;
        const dy = y1 - y0;
        ...
        for (let i = 0; i <= steps; i++) {
          const t = i / steps;
          points.push([x0 + t * dx, y0 + t * dy]);
        }
        return points;
      };

      context.fillStyle = "rgb(0, 0, 50)"; // Set the fill color
      ...
      while (stack.length > 0) {
        ...
        if (px < 0 || px >= context.canvas.width) continue;
        if (py < 0 || py >= context.canvas.height) continue;
        if (((x - px) ** 2 + (y - py) ** 2) ** 0.5 > fillRange) continue;

        const color = getPixelColour(px, py);
        if (color.a === 0) continue;
        if (color.r === 0 && color.g === 0 && color.b === 50) continue;

        const interpolatedPoints = interpolate(prevX, prevY, px, py, 1);
        for (const [ix, iy] of interpolatedPoints) {
          drawCircle(ix, iy, radius);
        }
  ...
  return (
    <div>
      ...
      {fillComplete && <div>Fill Complete!</div>}
    </div>
  );
};
